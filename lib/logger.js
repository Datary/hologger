...
Logger.prototype.init = function(){
    ...
    this.configure(new LocalStreamer());
    this.enable("Local");
    ...
    function demux(level){
        ...
        return function(){
            for (let streamer in streamers) {
                if (this._enabledStreamers.has(streamer)) return;
...
Logger.prototype.attach = function(isil, library){
    ...
    if (arguments.length === 1) {
        library = arguments[0];
        isil = null;
        console.warn("[HOLOGGER] ISIL not provided. Current library codes may get overwritten during attach.");
    } else if (typeof isil !== "string") {
        console.warn("[HOLOGGER] ISIL provided is not {String} and will be ignored. Current library codes may get overwritten during attachment.");
        isil = null;
    }
...
Logger.prototype.use = function(isil){
    if (typeof isil !== "string") {
        console.warn("[HOLOGGER] ISIL provided is not {String} and will be ignored. Global library will be used.");
    } else if (!this._library[isil]) {
        console.warn("[HOLOGGER] ISIL provided is not available in the library and will be ignored. Global library will be used.");
    } else {
