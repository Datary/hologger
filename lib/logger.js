 ...
 */
Logger.prototype.init = function(library, isil){
    //streamer pre-definido
    this._streamers.local =  new LocalStreamer(library, isil);
    this._streamer = this._streamers.local;
    this._strategy = "Local";


    //streamers opcionales
    process.env = process.env || {};
    if (process.env.LOGENTRIES_TOKEN) {
        this._streamers.logentries = new LogentriesStreamer(library, isil);
        this._streamer = this._streamers.logentries;
        this._strategy = "Logentries";
    }
    if (process.env.APPINSIGHTS_INSTRUMENTATIONKEY)  {
        this._streamers.applicationInsights = new ApplicationInsightsStreamer(library, isil);
        this._streamer = this._streamers.applicationInsights;
        this._strategy = "ApplicationInsights";
    }
