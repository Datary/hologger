...
function Logger(library, isil){
    ...
    this._isil = isil || null;      //vid @description
...
Logger.prototype.init = function(){
    ...
    //distribuye la peticion de logueo a todos los {Streamer} habilitados
    function demux(level){
        return function(){
            ...
            var isil = this._isil;
            for (let st in streamers) {
                if (!enabledStreamers.has(st)) return;
                else streamers[st][level].call(streamers[st], library, isil, ...arguments);
            }
        };
    }
};
...
Logger.prototype.attach = function(){
    ...
    if (arguments.length === 1) {
        if (typeof arguments[0] === "number") {
            ...
            isil = null;
            try {
                library = require(arguments[0]);
            } catch (e) {
                path = findUp.sync('logger.json', {cwd: arguments[0]});
                if (path) library = require(path);
                else {
                    let msg = "[HOLOGGER] Library not found in filesystem,";
    ...
    if (arguments.length > 1) {
        ...
        if (typeof arguments[1] === "string") {
            try {
                library = require(arguments[1]);
            } catch (e) {
                path = findUp.sync('logger.json', {cwd: arguments[1]});
                if (path) library = require(path);
                else {
                    let msg = "[HOLOGGER] Library not found in filesystem,";
                    msg += " up from " + arguments[1] +".";
                    console.warn(msg);
                    library = {};
                }
            }
...
Logger.prototype.use = function(isil){
    if (typeof isil !== "string" && typeof isil !== "number") {
        ...
        let msg = "[HOLOGGER] ISIL " + isil + " selected for use is not available in the library. ";
        msg += "Global library will be used instead.";
        console.warn(msg);
    } else {
