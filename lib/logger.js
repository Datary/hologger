...
Logger.prototype.init = function(library, isil){
    ...
    this.enable(new LocalStreamer(library, isil));
    this.use("Local");
    ...
    if (process.env.LOGENTRIES_TOKEN) {
        this.enable(new LogentriesStreamer(library, isil));
        this.use("Logentries");
    }
    if (process.env.APPINSIGHTS_INSTRUMENTATIONKEY)  {
        this.enable(new ApplicationInsightsStreamer(library, isil));
        this.use("ApplicationInsights");
    }
 ...
 * @return {Logger} for chaining
 */
Logger.prototype.use = function(name) {
  if (!name) throw new Error('A name is required in order to set a Streamer as default');
  if (!this._streamers[name]) throw new Error('Streamer ' + name + ' have not been enable yet');
